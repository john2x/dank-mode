...
- [ ] Add highlighting of comment under point
- [ ] Navigation methods for comment tree
  - [ ] Move between comments directly above/below
  - [ ] Jump to parent
  - [ ] Jump to next parent
  - [ ] Jump between siblings
- [ ] Interact with 'load more' comments.
  - load in new buffer? or use narrowing?
  - narrowing would be better because it can keep the comments within the main buffer
  - for narrowing to work, need to add hooks to remove the first level of indentation
- [ ] Start folding comment trees.
- [ ] Command to load a user's subscriptions. This can be used to for
